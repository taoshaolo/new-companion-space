<template>
  <van-cell-group>
    <van-cell
      ...
      <template #title>
        <van-text-ellipsis style="margin-left: 20px;font-weight: bold" :content="blog.title" rows="1"/>
      </template>
      <template #value>
      ...
      <template #label>
        <span style="margin-left: 10px">{{ moment(blog.createTime).format('YYYY-MM-DD HH:mm:ss') }}</span>
      </template>
    </van-cell>
  </van-cell-group>
</template>
