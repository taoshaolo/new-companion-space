<template>
  ...
  <van-tabbar v-model="active" v-if="!NOT_SHOW_NAVBAR_AND_TABBAR.includes(route.path)" @change="onChange" route>
    <van-tabbar-item icon="home-o" name="index" replace to="/">推荐</van-tabbar-item>
    <van-tabbar-item icon="contact-o" name="friends" :badge="applyFriendsCount<=0?null:applyFriendsCount" replace
                     to="/friends">好友
    </van-tabbar-item>
    <van-tabbar-item icon="chat-o" name="chat" replace to="/public_chat">聊天厅</van-tabbar-item>
    <van-tabbar-item icon="friends-o" name="team" replace to="/team">队伍</van-tabbar-item>
    <van-tabbar-item icon="user-circle-o" name="user" replace to="/user">我的</van-tabbar-item>
  </van-tabbar>
</template>
